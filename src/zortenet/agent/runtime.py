...
A deliberately small ReAct-style tool-calling loop over the :mod:`zortenet.llm` provider
abstraction and a :class:`~zortenet.agent.tools.ToolRegistry`. It is the execution engine behind
...
- **Observable** — every run can be captured by a :class:`~zortenet.agent.trajectory.TrajectoryLogger`
