...
Usage::
    ...
    python -m zortenet.packs.new my-pack --description "What it does"

Generates ``src/zortenet/packs/my_pack/__init__.py`` (PACK metadata + a working example tool)
...
(Scaffolded by `python -m zortenet.packs.new`. See CONTRIBUTING.md for the pack contract:
...
from zortenet.core.bus import EventStore
from zortenet.packs.{snake} import PACK, build_registry
...
def generate(name: str, description: str, root: Path) -> dict:
    ...
    pack_dir = root / "src" / "zortenet" / "packs" / snake
    ...
    registration = f'    "{name}": "zortenet.packs.{snake}",'
    return {
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="zortenet-new-pack")
    ...
    print("\nregister it in src/zortenet/packs/__init__.py PACK_MODULES:")
