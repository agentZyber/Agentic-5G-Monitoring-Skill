...
Logging is opt-in via constructor or the ``CORELAB_TRAJECTORIES`` environment variable —
...
class TrajectoryLogger:
    ...
    def from_env(cls, default_dir: Optional[str] = None) -> Optional["TrajectoryLogger"]:
        """Build a logger from ``CORELAB_TRAJECTORIES``.
        ...
        """
        raw = os.getenv("CORELAB_TRAJECTORIES")
        if raw is None:
