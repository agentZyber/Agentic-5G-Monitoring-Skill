"""CORE Lab NCSRD toolkit app — one capability core, every protocol face, in one FastAPI process.
...
at ``/mcp``) → trajectory capture (on by default here; opt out with ``CORELAB_TRAJECTORIES=off``).
...
Run:  uvicorn corelab.app:app --port 5001          (module-level default app)
      python -m corelab.interop.mcp_server          (stdio MCP face for local clients)
"""
...
from corelab import __version__
from corelab.agent.runtime import DEFAULT_SYSTEM_PROMPT, AgentRuntime
from corelab.agent.tools import ToolRegistry
from corelab.agent.trajectory import TrajectoryLogger
from corelab.connectors.base import catalog as connector_catalog
from corelab.core.bus import EventBus
from corelab.core.events import NetworkEvent
from corelab.intent.ledger import IntentLedger, IntentTransitionError
from corelab.interop.a2a_server import make_a2a_router
from corelab.interop.a2a_tasks import A2ATaskManager, make_a2a_jsonrpc_router
from corelab.interop.acp import make_acp_router
from corelab.interop.agntcy import oasf_record
from corelab.interop.agui import make_agui_router
from corelab.interop.anp import agent_description, did_document
...
def _executor_from_env():
    ...
    kind = os.getenv("CORELAB_EXECUTOR", "simulated").lower()
    if kind == "amarisoft":
        from corelab.connectors.amarisoft import AmarisoftClient, AmarisoftExecutor

        return AmarisoftExecutor(AmarisoftClient())
    from corelab.packs.intent_to_network import SimulatedExecutor

    return SimulatedExecutor()


def _packs_from_env() -> List[str]:
    raw = os.getenv("CORELAB_PACKS", ",".join(DEFAULT_PACKS))
...
def create_app(
    ...
    if "multi-agent-noc" in {p.strip().lower() for p in pack_names}:
        from corelab.multiagent.noc import build_default_specialists
    ...
    if enable_mcp:
        try:
            from corelab.interop.mcp_server import make_streamable_http_app
    ...
    app = FastAPI(
        title="CORE Lab NCSRD — Agentic 5G/6G Toolkit",
        version=__version__,
        lifespan=lifespan,
    )
    ...
    a2a_manager = A2ATaskManager(runtime, agent_name="corelab-5g")
    ...
    public_url = os.getenv("CORELAB_PUBLIC_URL", "http://localhost:5001")
...
# Default ASGI entrypoint: `uvicorn corelab.app:app`
app = create_app()
